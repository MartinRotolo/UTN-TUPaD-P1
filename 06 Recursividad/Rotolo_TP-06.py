...
def fibo_recur(posicion):
        if posicion == 0:
            return 0
        elif posicion == 1:
            return 1
        else:
            return fibo_recur(posicion-1)+fibo_recur(posicion-2)

num = int(input("Ingrese un numero entero: "))
for i in range(num):
        print(f"en la posicion {i} el valor de serie Fibonacci es {fibo_recur(i)}")
...
print(f"{base}^{exponente} = {calc_potencia(base, exponente)}")
...
print(f"El numero {num} en base decimal equivale a {decimal_a_binario(num)} en binario")
...
num = int(input("Ingrese un numero entero positivo: "))
print(f"El total de bloques de una piramide con {num} bloques en la base es = {contar_bloques(num)}")
